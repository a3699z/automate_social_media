...
const Common = {
  ...
  get_phone_number: async function (contact_id, phone_numbers) {
    var res = await new Promise(async (resolve, reject) => {
      db_connect.query(
        `SELECT *  FROM sp_whatsapp_phone_numbers WHERE pid = '` +
          contact_id +
          `' AND phone NOT IN( ? ) LIMIT 5`,
        [phone_numbers],
        (err, res) => {
          return resolve(res);
        }
  ...
  get_instance: async function (instance_id) {
    var res = await new Promise(async (resolve, reject) => {
      ...
      db_connect.query(
        "SELECT * FROM sp_whatsapp_sessions WHERE ?",
        data,
        (err, res) => {
          return resolve(res);
        }
  ...
  get_accounts: async function (accounts) {
    var res = await new Promise(async (resolve, reject) => {
      db_connect.query(
        "SELECT count(*) as count FROM sp_accounts WHERE id IN  (" +
          accounts +
          ") AND status = 1",
        (err, res) => {
          return resolve(res);
  ...
  update_status_instance: async function (instance_id, info) {
    var res = await new Promise(async (resolve, reject) => {
      ...
      db_connect.query(
        "UPDATE sp_whatsapp_sessions SET ? WHERE ?",
        data,
        (err, res) => {
          return resolve(res, true);
        }
  ...
  db_update_account: async function (
    instance_id,
    team_id,
    wa_info,
    account_id
  ...
  db_insert_stats: async function (team_id) {
    var res = await new Promise(async (resolve, reject) => {
      ...
      db_connect.query(
        "INSERT INTO sp_whatsapp_stats SET ?",
        data,
        (err, res) => {
          return resolve(res, true);
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (phone != "") {
        ...
        if (
          phone.startsWith("52") &&
          phone.length == 12 &&
          phone.substring(2, 3) != "1"
        ) {
          phone =
            phone.substring(0, 2) + "1" + phone.substring(2, phone.length);
        }

        if (phone != current_phone) {
          updateQuery =
            "UPDATE `sp_whatsapp_phone_numbers` SET  `phone`='" +
            phone +
            "'  WHERE `phone`=" +
            current_phone;
  ...
  post_type: function (mime, type) {
    ...
    if (type == 1) {
      ...
      } else if (mime == "audio/mpeg" || mime == "audio/ogg") {
