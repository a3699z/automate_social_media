...
class AuthModel extends Model
{
	...
	public function wo_login() {
		$code = $_GET['code'];
        $app_id = env('ABN_APP_ID');
        $app_secret = env('ABN_APP_SECRET');
        $abn_url = env('ABN_URL');
        $url = $abn_url.'/authorize?app_id='.$app_id.'&app_secret='.$app_secret.'&code='.$code;
        ...
        if (is_array($wo_json_reply) && isset($wo_json_reply['access_token'])) {
            ...
            $type = 'get_user_data';
            $url = $abn_url.'/api_request?access_token='.$access_token.'&type='.$type;
            ...
            if (is_array($user_data_array) && !empty($user_data_array) && isset($user_data_array['user_data'])) {
				...
				$user = db_get("*", TB_USERS, ["email" => $user_data['email']]);
                ...
                if (empty($user)) {
					...
					$avatar = save_img( get_avatar($full_name), WRITEPATH.'avatar/' );
                    ...
                    $data = [
                        ...
                        "password" => password_hash( $user_data['email'], PASSWORD_DEFAULT ),
                        "plan" => $plan,
						"expiration_date" => $expiration_date,
					...
					$user = db_get("id,status,ids", TB_USERS, "( email = '".$user_data['email']."' )");
					validate("empty", __("The account you entered does not match any account"), $user);
					$team = db_get("id,ids", TB_TEAM, "owner = '{$user->id}'");
					validate("empty", __("The is a problem on your account. Please try again later"), $team);
					if($user->status == 1){
